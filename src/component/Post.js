...
export class Post extends Component {
  ...
  render() {
    ...
    return Object.keys(curr_post).length === 0 ? (
      <NotFound />
    ) : (
      <Container>
        <Header></Header>
        ...
        <Row style={{ margin: "10px" }}>
          <Col xs={12} style={{ backgroundColor: "#e6efff" }}>
            <p>{curr_post.body}</p>
          </Col>
          <Col xs={12}>
...
const mapDispatchToProps = dispatch => ({
  ...
  addNewComment: (newCommentBody, cb) =>
    dispatch(addNewComment(newCommentBody, cb))
});
